...
def main():
    ...
    parser.add_argument("-m", "--masked_statute_output_dir",
                        help="Path to save new masked data")
    ...
    for fl in os.listdir(args.input_dir):
        ...
        data, n_sections = mask_statutes(data, section_info)

        with open(os.path.join(args.masked_statute_output_dir,
                               f"{flname}.txt"), 'w') as f:
            f.write(data)
