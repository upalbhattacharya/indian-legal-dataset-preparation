...
def main():
    ...
    parser.add_argument("-m", "--min", type=int, default=5,
                        help="Minimum number of sentences required")
    ...
    set_logger(os.path.join(args.log_path, "get_roles_sentences"))
    logging.info("Inputs:")
    ...
    for fl in os.listdir(args.input_path):
        ...
        if len(sents) < args.min:
            logging.info(f"{flname} has only {len(sents)} relevant "
                         "sentences. Skipping")
            del role_lengths[flname]
            continue
