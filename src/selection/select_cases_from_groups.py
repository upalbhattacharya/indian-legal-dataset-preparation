...
def main():
    ...
    parser.add_argument("-c", "--conditions", type=str,
    ...
    parser.add_argument("-a", "--act_areas",
                        help="Area information for acts")
    parser.add_argument("-aam", "--area_min", type=int, default=None,
    ...
    if args.conditions is not None:
        ...
        logging.info(f"Parsed areas, acts, chapters and sections: {targets_dict}")
        ...
        for case, info in case_info.items():
            ...
            logging.info(f"Found relevant acts {rel_acts}")
            ...
            if targets_dict["acts"] == []:
                if targets_dict["chapters"] == []:
                    if targets_dict["sections"] == []:
                        rel_sections = [sec
                                        for sec in sections
                                        if set(act_areas.get(
                                            sec.split("_")[0], "")).intersection(
                                                set(rel_areas))
                                        != set()]
                    else:
                        rel_sections = [sec
                    ...
                    rel_chapters = [section_chapters.get(sec, "")
                                    for sec in rel_sections
                                    if section_chapters[sec] in
                                    targets_dict["chapters"]]

                    rel_chapters = list(filter(None, rel_chapters))

                rel_acts = [sec.split("_")[0] for sec in rel_sections]

            elif targets_dict["chapters"] == []:
                if targets_dict["sections"] == []:
                    rel_sections = [sec
                                    for sec in sections
            ...
            new_info = {"areas": order(Counter(rel_areas)),
        ...
        area_case_num = {
                k: len(v) for k, v in sorted(area_case_info.items(),
    ...
    if args.area_min is not None:
        prune_areas = [area for area, num in area_case_num.items()
                       if num < args.area_min]
        logging.info(f"Pruning the following areas: {prune_areas}")
        select_case_info = clean_min_areas(select_case_info, prune_areas,
                                           act_areas)
    ...
    with open(os.path.join(args.output_dir,
                           "case_area_act_chapter_section_info.json"),
              'w') as f:
    ...
    with open(os.path.join(args.output_dir, "area_case_info.json"),
              'w') as f:
    ...
    with open(os.path.join(args.output_dir, "area_case_num.json"),
              'w') as f:
...
def clean_min_areas(select_case_info: dict, prune_areas: list,
                    act_areas: dict) -> dict:
    """
...
def clean_min_acts(select_case_info: dict, prune_acts: list,
                   act_areas: dict) -> dict:
    """
...
def clean_min_chapters(select_case_info: dict, prune_chapters: list,
...
def clean_min_sections(select_case_info: dict, prune_sections: list,
                       section_chapters: dict, act_areas: dict) -> dict:
    """
